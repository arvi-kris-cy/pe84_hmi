...
The following example clones the "**Local Voice**" application with the desired name "LocalVoice" configured for the *KIT_PSE84_EVAL_EPC4* BSP or *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/afe*:
<br> eg)
<br>

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC4 --app-id mtb-example-psoc-edge-afe-application --user-app-name AFEApplication --target-dir "C:/afe"
   ```
...
In common.mk, ensure `CONFIG_INFERENCING_LIB` is set to `ITSI_M55_U55` or `SENSORY` (Sensory is supported only in Mains powered configuration). If is is set to `DISABLED`, then inferencing will not happen in CM55.
