/******************************************************************************
* File Name : battery_powered_afe.c
*
* Description :
* Source file for Audio Front End in CM55 application. (battery powered use-case)
********************************************************************************
...
#define APP_BUF_INFO_QUEUE_COUNT                       (200u)
...
QueueHandle_t app_buf_info_queue = NULL;

cy_buffer_pool_handle input_buf_handle;
...
static cy_rslt_t afe_output_callback(cy_afe_t handle, cy_afe_buffer_info_t *output_buffer_info, void *user_arg)
{
    ...
    {
        battery_powered_inference_feed((short*)output_buffer, buffer_info);
    }
...
/*******************************************************************************
* Function Name: battery_powered_afe_mw_init
********************************************************************************
* Summary:
* Initialize AFE MW.
*
...
int battery_powered_afe_mw_init()
{
    ...
    app_buf_info_queue = xQueueCreate(APP_BUF_INFO_QUEUE_COUNT, sizeof(int32_t));
    if (app_buf_info_queue == NULL)
    {
        app_afe_log("Failed to initialize queue to store buffer info");
        return AFE_APP_FAILURE;
    }
...
/*******************************************************************************
* Function Name: battery_powered_afe_mw_feed
********************************************************************************
* Summary:
*  Feed audio data to AFE middleware.
*
*******************************************************************************/

int battery_powered_afe_mw_feed(void* audio_input, void* aec_buffer, cy_svc_buffer_info_t buffer_info)
{
    cy_rslt_t result = CY_RSLT_SUCCESS;
    cy_svc_buffer_info_t buf_info = buffer_info;

    if (pdTRUE != xQueueSendToBack(app_buf_info_queue, &buf_info, portMAX_DELAY))
    {
        app_log_print("AFE failed to push buffer info to queue \r\n");
        return AFE_APP_FAILURE;
    }
    if (is_inference_license_valid() == -1) 
    {
        app_log_print("Invalid Inference library license \r\n");
        license_limitation_exit();
    }
    result = cy_afe_feed(afe_handle, audio_input, aec_buffer);
