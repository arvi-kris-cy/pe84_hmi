/******************************************************************************
* File Name : tune_afe_mains_powered.c
*
* Description :
...
#include "tune_afe_mains_powered.h"
...
static cy_rslt_t afe_output_callback(cy_afe_t handle, cy_afe_buffer_info_t *output_buffer_info, void *user_arg)
{
    ...
    usb_send_out_dbg_put(USB_CHANNEL_2,(int16_t *)output_buffer);
...
cy_rslt_t app_afe_alloc_memory(cy_afe_mem_id_t mem_id, uint32_t size, void **buffer)
{
      ...
      *buffer = NULL;
      ...
      switch(mem_id)
      {
          ...
          {
              app_log_print("DSNS Memory requires %ld bytes in heap \r\n",(long)size);
          ...
          {
              app_log_print("Persistent Memory (DTCM) requires %ld bytes \r\n",(long)size);
          ...
          {
              app_log_print("Scratch Memory (DTCM) requires %ld bytes \r\n",(long)size);
...
/*******************************************************************************
* Function Name: tune_mains_afe_mw_init
********************************************************************************
* Summary:
* Initialize AFE MW for mains powered application. (Tuning)
*
* Parameters:
*  None
* 
...
int tune_mains_afe_mw_init()
{
    ...
    /* tuner callbacks */
    tuner_cb.notify_settings_callback = afe_tuner_settings_notify_cb;
    tuner_cb.read_request_callback = afe_tuner_read_req_cb;
    tuner_cb.write_response_callback = afe_tuner_write_res_cb;
