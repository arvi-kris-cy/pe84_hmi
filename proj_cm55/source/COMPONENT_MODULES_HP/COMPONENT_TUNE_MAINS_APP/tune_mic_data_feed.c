...
*
* Description :
* Feeds data received via PDM ISR/USB ISR to the audio pipeline on CM55 for Tuning
********************************************************************************
...
int16_t aec_ref_buffer[MONO_AUDIO_DATA_IN_BYTES] __attribute__ ((section(".cy_socmem_data"))) = { 0 };
...
void usb_mic_data_feed(int16_t *audio_data)
{
 ...
 /* Feed the data to AFE */
    result = mains_powered_afe_mw_feed((int16_t*)audio_data, aec_reference);
