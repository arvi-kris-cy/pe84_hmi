...
#if defined(ENABLE_IFX_SOD) || defined(ENABLE_IFX_LPWWD) || defined(ENABLE_IFX_PRE_PROCESS_HPF) || defined(ENABLE_IFX_ITSI) || defined(ENABLE_IFX_FE)
#include "ifx_sp_utils_priv.h"
#endif
#include "ifx_pre_post_process.h"
#if defined(ENABLE_IFX_HPF) || defined(ENABLE_IFX_BF) || defined(ENABLE_IFX_DVRB) || defined(ENABLE_IFX_AEC) || defined(ENABLE_IFX_NS) || \
    defined(ENABLE_IFX_ES) || defined(ENABLE_IFX_ANALYSIS) || defined(ENABLE_IFX_SYNTHESIS)
...
int32_t ifx_sp_enh_profile_init(void* modelPt, int32_t config, ifx_sp_profile_cb_fun cb_func, void* cb_arg)
{
    ...
    //Warnings fix, variable is enabled under conditional algo inclusions.
    (void)app_enable;
    (void)comp_enable;
...
int32_t ifx_pre_post_profile_init(void* modelPt, int32_t ip_id, uint8_t enable, ifx_sp_profile_cb_fun cb_func, void* cb_arg)
{
    ...
    case IFX_PRE_PROCESS_IP_COMPONENT_SOD:
    {
        ...
        dPt->sod_component.profile.profile_config = enable;
        cycle_profiler_init(&(dPt->sod_component.profile));
        break;
    }
#endif
#ifdef ENABLE_IFX_PRE_PROCESS_HPF
    case IFX_PRE_PROCESS_IP_COMPONENT_HPF:
    {
        preprocess_hpf_top_struct* dPt = (preprocess_hpf_top_struct*)modelPt;

        dPt->pre_proc_hpf.profile.profile_config = enable;
        cycle_profiler_init(&(dPt->pre_proc_hpf.profile));
        break;
    }
#endif
    ...
    {/* MFCC and LOG_MEL has common data structure */
        ...
        dPt->spctrg_component.profile.profile_config = enable;
        cycle_profiler_init(&(dPt->spctrg_component.profile));
        break;
    }
#endif
    ...
    case IFX_POST_PROCESS_IP_COMPONENT_HMMS:
    {
        ...
        dPt->pp_component.profile.profile_config = enable;
        cycle_profiler_init(&(dPt->pp_component.profile));
        break;
    }
#endif
#ifdef ENABLE_IFX_ITSI
    case IFX_PRE_PROCESS_IP_COMPONENT_DENOISE:
    {
        itsi_feature_top_struct* dPt = (itsi_feature_top_struct*)modelPt;

        dPt->feature_extrct.profile.profile_config = enable;
        cycle_profiler_init(&(dPt->feature_extrct.profile));
        break;
    }
    case IFX_PRE_PROCESS_IP_COMPONENT_AMPREDICT:
    {
        itsi_am_top_struct* dPt = (itsi_am_top_struct*)modelPt;

        dPt->am_predict.profile.profile_config = enable;
        cycle_profiler_init(&(dPt->am_predict.profile));
        break;
    }
    case IFX_POST_PROCESS_IP_COMPONENT_DFWWD:
    {
        itsi_dfwwd_top_struct* dPt = (itsi_dfwwd_top_struct*)modelPt;

        dPt->dfwwd.profile.profile_config = enable;
        cycle_profiler_init(&(dPt->dfwwd.profile));
        break;
    }
    case IFX_POST_PROCESS_IP_COMPONENT_DFCMD:
    {
        itsi_dfcmd_top_struct* dPt = (itsi_dfcmd_top_struct*)modelPt;

        dPt->dfcmd.profile.profile_config = enable;
        cycle_profiler_init(&(dPt->dfcmd.profile));
        break;
    }
#endif
    default:
...
int32_t ifx_pre_post_profile_print(void* Pt, int32_t ip_id)
{
    ...
    case IFX_PRE_PROCESS_IP_COMPONENT_SOD:
    {
        lPt = ((sod_top_struct *) Pt)->sod_component.profile;
        break;
    }
#endif
#ifdef ENABLE_IFX_PRE_PROCESS_HPF
    case IFX_PRE_PROCESS_IP_COMPONENT_HPF:
    {
        lPt = ((preprocess_hpf_top_struct*)Pt)->pre_proc_hpf.profile;
        break;
    }
#endif
#if defined(ENABLE_IFX_LPWWD) || defined(ENABLE_IFX_FE)
    ...
    case IFX_PRE_PROCESS_IP_COMPONENT_LOG_MEL:
    {
        lPt = ((spectrogram_top_struct *) Pt)->spctrg_component.profile;
        break;
    }
#endif
#ifdef ENABLE_IFX_LPWWD
    case IFX_POST_PROCESS_IP_COMPONENT_HMMS:
    {
        lPt = ((postprocess_top_struct*)Pt)->pp_component.profile;
        break;
    }
#endif
#ifdef ENABLE_IFX_ITSI
    case IFX_PRE_PROCESS_IP_COMPONENT_DENOISE:
    {
        lPt = ((itsi_feature_top_struct*)Pt)->feature_extrct.profile;
        break;
    }
    case IFX_PRE_PROCESS_IP_COMPONENT_AMPREDICT:
    {
        lPt = ((itsi_am_top_struct*)Pt)->am_predict.profile;
        break;
    }
    case IFX_POST_PROCESS_IP_COMPONENT_DFWWD:
    {
        lPt = ((itsi_dfwwd_top_struct*)Pt)->dfwwd.profile;
        break;
    }
    case IFX_POST_PROCESS_IP_COMPONENT_DFCMD:
    {
        lPt = ((itsi_dfcmd_top_struct*)Pt)->dfcmd.profile;
        break;
    }
#endif
    default:
...
#if 0
__weak; int platform_profile_get_tsc(uint32_t* val)
{
    (void)val;
    return 0;
}
